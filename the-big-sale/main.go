...
func matchSales(budget float64, items []SaleItem) []SaleItem {
  matched := make([]SaleItem, 0)
  for _, i := range items {
    if i.ReducedPrice > budget {
      continue
    }

		i.SalePercentage = float64((i.OriginalPrice-i.ReducedPrice)/i.OriginalPrice) * 100

    matched = append(matched, i)
	}

  sort.Slice(matched, func(i, j int) bool {
    return matched[i].ReducedPrice < matched[j].ReducedPrice
  })
